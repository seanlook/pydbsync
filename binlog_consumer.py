...
# when to start
# durable
# minimal mode: dml with unique key
# logger
    ...
    def init_dml_tmpl_part1(self):
        ...
        if self.row_type in (1, 2):  # insert or delete
            ...
            for row_c, row_v in self.row_data.items():
                ...
                # spec.1
                elif row_v is None and row_c in ['f_company_province', 'f_company_city']:
                    row_value.append('0')
                else:
            ...
            for row_c, row_v in self.row_data.items():
                ...
                # spec.1
                elif row_c in ['f_company_province', 'f_company_city'] and row_v is None:
                    template_param.append('`{0}`="0" /* %s */'.format(row_c))  # a is None
...
class MaxwellSync(object):
    ...
    def process_data(self, row):
        ...
        ## ddl statement
        if rowdata_sql.row_type == 4 and rowdata_sql.row_db == 'd_ec_crm':
            ...
            # broadcast ddl
            sql_str_list = []
            for ids in self.corpmod_ids:
                dbtable_mod = ' %s.%s ' % (rowdata_sql.row_db + str(ids), rowdata_sql.row_tb)
                sql_str_list.append(alter_sql.replace(dbtable, dbtable_mod, 1))

            logger.debug("statement ddl: %s", "; ".join(sql_str_list))
            self.dbconn_cur.query("; ".join(sql_str_list))
            # cur.close()
            return 1

        ## dml statement

        # convert database name: d_ec_crm -> d_ec_crm0
        row_db_new = rowdata_sql.row_db
        corp_id = rowdata_sql.row_data.get('f_corp_id', 0)
        if rowdata_sql.row_db == 'd_ec_crm':
            if corp_id != 0:
                corp_mod = rowdata_sql.row_data['f_corp_id'] % 128
                row_db_new += str(corp_mod)
            else:
                flag_row_db_all = 1
        ...
        if flag_row_db_all == 0:
            ...
            # broadcast non-f_corp_id table/data
            for ids in self.corpmod_ids:
                sql_str = rowdata_sql.generate_dml_template(rowdata_sql.row_db + str(ids))
    ...
    # rollbak mode
    def process_data_simple(self, row):
        """
        :param row: dict row data
        :return: -1: skip this message
                 1: success
        ...
        # sp.1
        if rowdata_sql.row_db in ['t_crm_chglog', 't_plan_log_step']:
            del rowdata_sql.row_data['f_id']
            # not replace into
        if rowdata_sql.row_data.get('__#alibaba_rds_row_id#__', 0):
            logger.info("column `__#alibaba_rds_row_id#__` in %s", rowdata_sql.row_data)
            del rowdata_sql.row_data['__#alibaba_rds_row_id#__']
    ...
    def binlog_sync(self):
        ...
        def callback(ch, method, properties, body):
            ...
            try:
                ...
                if not dbname_rewrite:
                    ret = self.process_data(data_row)
                else:
                    ret = self.process_data_simple(data_row)
